...
def solveBoxes(model, dict_variables, dict_equations, bxrd_options, solv_options,smpl_options=None):
    ...
    Args:
        ...
        :bxrd_options:      dictionary with user-settings for decompositon 
                            and scaling
    ...
    boxes = mostg.get_entry_from_npz_dict(bxrd_options["BoxReduction_fileName"], bxrd_options["redStep"]) 
    
    mainfilename = bxrd_options["fileName"] 
    npzName = bxrd_options["fileName"]
    for cur_solver in solv_options["solver"]:
        npzName+=cur_solver
    npzName+=".npz"
    bxrd_options["mainfileName"] = mainfilename  
    bxrd_options["npzName"] = npzName  
    ...
    if solv_options["parallel_boxes"]:  #TODO: needs to be adjusted
        res = parallelization.solveBoxesParallel(model, boxes, dict_variables, dict_equations, 
                    bxrd_options, solv_options) 
    
    else:
        res = {}
        if "boxes" in bxrd_options.keys():
            for l in bxrd_options["boxes"]:
                res[l]  = solveOneBox(model, boxes, l, dict_variables, dict_equations, 
                        bxrd_options, solv_options, smpl_options)                
        else:
            for l in range(0, len(boxes)):
                res[l]  = solveOneBox(model, boxes, l, dict_variables, dict_equations, 
                        bxrd_options, solv_options, smpl_options)
    results.write_successful_results(res, bxrd_options, smpl_options, solv_options)           
    #store_results(res, boxes, bxrd_options)


def store_results(res, boxes, bxrd_options):
    if "sampling" in bxrd_options.keys():
        mostg.store_list_in_npz_dict(bxrd_options["npzName"], results, 0, allow_pickle=True)
        return
    if "boxes" in bxrd_options.keys():#isinstance(bxrd_options["boxes"],list):
        boxNo = len(bxrd_options["boxes"])
    else:
        boxNo = len(boxes)
    for l in range(0, boxNo):
        if "boxes" in bxrd_options.keys(): k = bxrd_options["boxes"][l]
        else: k = l
        mostg.store_list_in_npz_dict(bxrd_options["npzName"], results['%d'%k], l, allow_pickle=True)


def solveOneBox(model, boxes, l, dict_variables, dict_equations, bxrd_options, solv_options, smpl_options=None):
    model.xBounds = [boxes[l]]
    bxrd_options["box_ID"] = l

    return solveBlocksSequence(model, solv_options, bxrd_options, smpl_options)


def sample_multiple_solutions(model, b,  smpl_options, 
    ...
    Args:
        ...
        :smpl_options:  user-specified dictionary with sampling settings
    ...
    for x in subSolutions:
        ...
        res_blocks,  solved, model = sample_and_solve_one_block(model, b, smpl_options, bxrd_options, solv_options,
                                            res_blocks)
        if solved: 
...
def sample_and_solve_one_block(model, b, smpl_options, 
    ...
    if smpl_options["smplNo"] != -1: 
        onePointInit.set_to_midPoint({"Block": cur_block}, bxrd_options)
    ...
    if smpl_options["smplMethod"]== "optuna":
        #samples = [[moi.func_optuna_timeout(cur_block, 0, smpl_options, 
        #                               bxrd_options)]] 
        samples = [[moi.do_optuna_optimization_in_block(cur_block, 0, smpl_options, bxrd_options)]]
        print("The residual of the sample point is: ", 
              numpy.linalg.norm(cur_block.getFunctionValues()))   

    else: 
       moi.sample_box_in_block(cur_block, 0, smpl_options, bxrd_options, samples)
...
def solve_samples_block(model, block, b, samples, solv_options, bxrd_options, res_blocks):
...
def solve_block(model, block, b, solv_options, bxrd_options, res_blocks):
    ...
    Args:
        ...
        :bxrd_options:  dictionary with structural settings of model
        :res_blocks:    dictionary with results from all model blocks
    ...
    """
    if bxrd_options["scaling"] != 'None': getInitialScaling(bxrd_options, 
                                model, block)
    res_solver = startSolver(block, b, solv_options, bxrd_options)  
...
def sort_results_from_solver_to_block(block, b, res_solver, res_blocks, bxrd_options):
...
def update_block_solutions(block, bxrd_options):
    ...
    if not block.FoundSolutions == []:
        for solution in block.FoundSolutions:
            if numpy.allclose(block.x_tot[block.colPerm], solution, atol=bxrd_options["absTol"]):
                return i 
            i+=1
...
def solveSamples(model, initValues, dict_equations, dict_variables, solv_options, bxrd_options):
    ...
    for k in range(0, len(initValues)): # TODO: Parallelization
        ...
        bxrd_options["sample_ID"] = k
        model.stateVarValues = [initValues[k]]    

        res_multistart['%d' %k] = solveSystem_NLE(model, dict_equations, dict_variables, solv_options, bxrd_options)

    return res_multistart

      
def solveSystem_NLE(model, dict_equations, dict_variables, solv_options, bxrd_options,smpl_options=None):
    ...
    if bxrd_options["scaling"] != 'None' and bxrd_options["scalingProcedure"] == 'tot_init':
        mos.scaleSystem(model, bxrd_options) 

            
    if bxrd_options["decomp"] == 'DM' or bxrd_options["decomp"] == 'None': 
        res_solver = solveBlocksSequence(model, solv_options, bxrd_options, smpl_options)
        updateToSolver(res_solver, dict_equations, dict_variables)
        return res_solver
    
    if bxrd_options["decomp"] == 'BBTF':
        i=0
        res_solver=[]
...
def solveBlocksSequence(model, solv_options, bxrd_options,
                        smpl_options=None):
    ...
    for b, bl in enumerate(model.all_blocks):
        if bxrd_options["debugMode"]: print("Block ", b, " is processed.")
        if "sampling" in bxrd_options.keys():
            #if b==0: and smpl_options["init_method"] == "tear_sampling":

            if len(model.stateVarValues) > 1: 
                res_blocks = sample_multiple_solutions(model, b, smpl_options, 
                                                       bxrd_options, solv_options, 
                                                       res_blocks)
            else:      
            ...
            res_blocks[b] = startSolver(cur_block, b, solv_options, bxrd_options)
...
def getInitialScaling(bxrd_options, model, curBlock):
    """solve block decomposed system in sequence. This method can also be used
    if no decomposition is done. In this case the system contains one block that 
    equals the entire system.
    ...
    if bxrd_options["scalingProcedure"] =='tot_init': # scaling of complete decomposed system
            curBlock.setScaling(model)
        
    elif bxrd_options["scalingProcedure"] =='block_init' or bxrd_options["scalingProcedure"] =='block_iter': # blockwise scaling
            mos.scaleSystem(curBlock, bxrd_options)   
...
def doNewton(curBlock, b, solv_options, bxrd_options, res_solver):
    ...
    Args:
        ...
        :bxrd_options:      dictionary with user specified structure settings
        :res_solver:        dictionary with results from solver
       
    """
    
    try: 
        exitflag, iterNo = newton.doNewton(curBlock, solv_options, bxrd_options)
...
def doScipyRoot(curBlock, b, solv_options, bxrd_options, res_solver):
...
def doScipyFsolve(curBlock, b, solv_options, bxrd_options, res_solver):
    """ starts scipy's fsolve method
...
def doScipyOptiMinimize(curBlock, b, solv_options, bxrd_options, res_solver):
    ...
    try: 
        exitflag, iterNo = scipyMinimization.minimize(curBlock, solv_options, bxrd_options)
        res_solver["IterNo"] = iterNo
        res_solver["Exitflag"] = exitflag
        res_solver["CondNo"] = numpy.linalg.cond(curBlock.getScaledJacobian())
        res_solver["FRES"] = curBlock.getFunctionValues()
...
def doipoptMinimize(curBlock, b, solv_options, bxrd_options, res_solver):
    """ starts minimization procedure from scipy
    ...
    try:
        ...
        exitflag, iterNo = ipoptMinimization.minimize(curBlock, solv_options, bxrd_options)
...
def do_casadi_ipopt_minimize(curBlock, b, solv_options, bxrd_options, res_solver):
    ...
    try:
        ...
        exitflag, iterNo = ipopt_casadi.minimize(curBlock, solv_options, bxrd_options)
        res_solver["IterNo"] = iterNo-1
        res_solver["Exitflag"] = exitflag
        res_solver["CondNo"] = numpy.linalg.cond(curBlock.getScaledJacobian())
        res_solver["FRES"] = curBlock.getFunctionValues()
...
def doMatlabSolver_mscript(curBlock, b, solv_options, bxrd_options, res_solver):
    ...
    try:
        exitflag, iterNo = matlabSolver.fsolve_mscript(curBlock, solv_options, bxrd_options)
        res_solver["IterNo"] = iterNo-1
        res_solver["Exitflag"] = exitflag
        if exitflag == 1:
            res_solver["CondNo"] = numpy.linalg.cond(curBlock.getScaledJacobian())
...
def doMatlabSolver(curBlock, b, solv_options, bxrd_options, res_solver):
    """ starts matlab runner for externam matlab file with fsolve
    CAUTION: Additional matlab file is required
    ...
    try:
        exitflag, iterNo = matlabSolver.fsolve(curBlock, solv_options, bxrd_options)
        res_solver["IterNo"] = iterNo-1
        res_solver["Exitflag"] = exitflag
        res_solver["CondNo"] = numpy.linalg.cond(curBlock.getScaledJacobian())
        res_solver["FRES"] = curBlock.getFunctionValues()
...
def sample_tear_block(model, bxrd_options, smpl_options, solv_options):
    b = len(model.all_blocks)
    res_blocks={}
    res_blocks, solved, model = sample_and_solve_one_block(model, b, smpl_options, 
                                                    bxrd_options, solv_options,
                                                    res_blocks)             
